...
export class Cookie implements CookieInterface {
  ...
  getItem(sKey: string): string {
    ...
    if (this.encryption && this.encryptionType) {
      res = this.encryptionType === 'des' ? CryptoJS.DES.decrypt(res, this.screctKey).toString(CryptoJS.enc.Utf8) : CryptoJS.AES.decrypt(res, this.screctKey).toString(CryptoJS.enc.Utf8)
    }
    return res;
  }
  setItem(
  ...
  keys(): Array<String> {
    ...
    for (var nIdx = 0; nIdx < aKeys.length; nIdx++) {
      let tVal = decodeURIComponent(aKeys[nIdx]);
      if (this.ns.length) {
        const idx = aKeys[nIdx].indexOf(".");
